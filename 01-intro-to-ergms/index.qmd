...
In more simple terms, ERG models resemble Logistic regressions. The term $\tpose{\thetas} s(\y)$ is the sum of the product between the parameters and the statistics (like you would see in a Logistic regression,) and its key component is the composition of the vector of sufficient statistics, $s(\y)$. The latter is what defines the model.
